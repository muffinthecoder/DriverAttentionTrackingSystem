...
def TakeImages(user_id, user_name, processor=None):
    ...
    try:
        ...
        if not (name.replace(" ", "").isalpha() and Id.strip() != ""):
        ...
        while sampleNum < 100:
            ...
            for (x, y, w, h) in faces:
                sampleNum += 1
                img_path = get_path(f"TrainingImage/{name}.{serial}.{Id}.{sampleNum}.jpg")
                cv2.imwrite(img_path, gray[y:y + h, x:x + w])
            if processor:
                # For Streamlit: update video frame if processor exists
                processor.frame = img
